...
- **x**: x变量
- **y**: y变量
- z: z变量，只接受时间/日期变量，并打开时间轴
- series: Series(系列)变量
- weight: 权重变量，可用于直方图、气泡图等
- type: 默认scatter，可选'scatter', 'bubble', 'bar', 'line', 'linesmooth', 'map', 'k', 'pie', 'ring', 'area', 'areasmooth', 'chord', 'force', 'tree', 'treemap', 'wordcloud', 'heatmap', 'histogram', 'funnel', 'pyramid', 'radar', 'radarfill'
...
- splitNumber: 如打开数据漫游，可指定数据范围切分段数，默认为连续漫游轴
- dataRangePalette: 如打开数据漫游，可单独指定漫游色板，否则采用Echarts默认值
- xlab: x轴标题
- ylab: y轴标题
...
- palette: 默认aetnagreen，使用调色板
    - 种类:
        ...
        - RColorBrewer palettes: 可用'BrBG', 'PiYG', 'PRGn', 'PuOr', 'RdBu', 'RdGy', 'RdYlBu', 'RdYlGn', 'Spectral', 'Accent', 'Dark2', 'Paired', 'Pastel1', 'Pastel2', 'Set1', 'Set2', 'Set3', 'Blues', 'BuGn', 'BuPu', 'GnBu', 'Greens', 'Greys', 'Oranges', 'OrRd', 'PuBu', 'PuBuGn', 'PuRd', 'Purples', 'RdPu', 'Reds', 'YlGn', 'YlGnBu', 'YlOrBr', 'YlOrRd'
    ...
    - 用法:
        - 可以不指定，使用函数默认
        - 可以`palette=NULL`，使用Echarts默认
        - 可以`palette=palette name`指定上述任何一种色板
        - 规定色板的同时跟个数限定，限定色板颜色的个数，如`palette='calc(3)'`，会从calc色板中随机取3种颜色
        - 可以`palette=c(color1,color2,color3,...)`自定义色板向量，向量可以是颜色名，也可以是Hex表达式。可以用`colors()`函数查看所有支持的颜色名称，`demo(colors)`查看颜色效果
...
```{r linesmooth}
a=echartR(dfiris, x = ~id, y= ~Value, series= ~Param, type='linesmooth',
        symbolList='none',
...
```{r map gdp data}
gdp <- readLines("C:/HMSProjects/Data Analytics/RWork/ChinaGDP.txt")
