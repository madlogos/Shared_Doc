...
- palette: 默认aetnagreen，使用调色板
    ...
    - 用法:
        ...
        - 规定色板的同时跟一个数字，可以限定色板颜色的个数，如`palette='calc(3)'`
        - 可以`palette=c(color1,color2,color3,...)`自定义色板向量，向量可以是颜色名，也可以是Hex表达式
...
echartR(player, x=~para, y=~value, series=~name, type='radarfill',
        symbolList=rep('none',2),
        title='Lahm vs Alves (by @mixedknuts)', palette=c('red','navy'))
```
...
```{r map gdp data}
    ...
    dtgdp$Prov <- as.factor(iconv(dtgdp$Prov,to='utf-8'))
...
echartR(worldgdp, x = ~country, y = ~GDP, type=c('map','world','area'),
        ...
        palette='rainbow(6)', dataRange=c("High","Low"))
```
