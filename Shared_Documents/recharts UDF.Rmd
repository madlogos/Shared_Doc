...
knitr::opts_chunk$set(message=FALSE,warning=FALSE,results='asis',cache=FALSE)
```
...
```{r unstack hbar,results='markup'}
echartR(data = dtiris, x = ~Param, y = ~Mean, series = ~Species, xyflip=T,
...
echartR(airquality, y=~Temp, type='histogram', splitNumber=13,
        palette='pastel2(1)', title='Histogram of temperature (airquality)')
```
...
线段平滑`type='areasmooth'`，并打开数据缩放`dataZoom=TRUE`
```{r areasmooth}
...
echartR(dfiris, x = ~id, y= ~Value, series= ~Param, type='areasmooth',stack=T,
        palette=c('red','yellow','lime','skyblue'), 
...
## Radar 雷达图
雷达图就是极坐标系下的线图，通过Echarts的`polar`参数模块控制。
...
echartR(dtgdp, x = ~Prov, y = ~GDP, series= ~Year, 
        ...
        dataRangePalette=c('red','orange','yellow','lime','green'),
