...
- markLine: 显示标线，默认不显示。格式写作一个列表 Show markline, default to NULL. The grammar is a list:
    - 缩略格式 Short form：list(c('data series 1 name|index','min|max|average|lm'),c('data series 2 name|index','min|max|average|lm'),...)。如list('male','average')或list(1,'average')都可表示male数据系列平均值标线，只用于line, linesmooth, bar, scatter, bubble。`lm`可出线性回归标线，只用于散点或气泡图。E.g., both list('male','average') and list(1,'average') could refer to an average markline of the series 'male', only available for line, linesmooth, bar, scatter, bubble charts. 'lm' refers to linear regresson markline which is only available for scatters and bubbles.
    - 完整格式 Full form: list(c('data series 1 name|index','value 1','x1 begin','y1 begin','x1 end','y1 end'),c('data series 2 name|index','value 2','x2 begin','y2 begin','x2 end','y2 end'),...)。如list(c('male',100,0,5,100,5))表示在'male'数据系列中画一条穿越P(0,5)和P(100,5)的直线。E.g., list(c('male',100,0,5,100,5)) refers to a markline through P(0,5) and P(100,5) as of sereis 'male'. 在line, bar, k, scatter图中，'x1 begin','y1 begin','x1 end','y1 end'均被理解为直角坐标系的定位。 In line, bar, k and scatter charts, 'x1 begin','y1 begin','x1 end','y1 end' are comprehended as coordinates.
...
没有数据系列，因此指定显示第1个系列的均数线(`markLine=list(c(1,'average'`)。
...
echartR(data = iris, x = ~Sepal.Width, y = ~Petal.Width, 
        ...
        markLine=list(c(1,"average")))
```

### Multi-series Scatter 多系列散点图

指定series，且显示范围从零点开始(`scale=FALSE`)。在第2个数据系列(versicolor)中打开最大(max)、最小(min)、均线(average)三根标线。
...
同时拟合3条回归线(markLine写作short form，类型指定为'lm')。
...
echartR(data = iris, x = ~Sepal.Width, y = ~Petal.Width, series = ~Species,
        ...
        title = 'Scatter - Sepal Width vs Petal Width, by Species',
        subtitle = '(source: iris)', xlab = 'Sepal Width', ylab = 'Petal Width',
        markLine = list(c(1,'lm'),c(2,'lm'),c(3,'lm')))
```
...
echartR(data = iris, x = ~Sepal.Width, y = ~Petal.Width, 
        ...
        subtitle = '(source: iris)',
        xlab = 'Sepal Width', ylab = 'Petal Width',
        markLine=list(c('Linear Coef.',round(fit$coefficients[[2]],2),
...
chinapm25 <- unlist(strsplit(paste0(readLines("China PM2.5.txt"),
                                    collapse=""),","))
chinapm25 <- as.data.frame(matrix(chinapm25,byrow=T,ncol=4),stringsAsFactors=F)
names(chinapm25) <- c("City","PM25","xcoord","ycoord")
echartR(chinapm25, x=~City, y=~PM25, xcoord=~xcoord, ycoord=~ycoord,
        type=c('map','china','point'))
```

## Wordcloud 词云
直接knitr时readLines读取网页会报错，只能将网页 'http://top.baidu.com/buzz?b=1' 的源代码保存到本地再读取。
...
```{r word cloud}
a=echartR(hotword[1:30,], x=~Keyword, y=~Freq, type="wordcloud", 
