...
- markLine: 显示标线，默认不显示。格式写作一个列表 Show markline, default to NULL. The grammar is a list:
    - 缩略格式 Short form (长度3或4 length 3 or 4)：
    ...
    如list('male',NA,'average')或list(1,NA,'average')都可表示male数据系列平均值标线，只用于line, linesmooth, bar, scatter, bubble。`lm`可出线性回归标线，只用于散点或气泡图。如list('male',NA,'average',T)则表示male系列开启炫光特效。
    ...
    - 完整格式 Full form (长度7或8 length 7 or 8): 
    ...
    如list(c('male',NA,100,0,5,100,5))表示在'male'数据系列中画一条穿越P(0,5)和P(100,5)的直线。E.g., list(c('male',NA,100,0,5,100,5)) refers to a markline through P(0,5) and P(100,5) as of sereis 'male'. 在line, bar, k, scatter图中，'x1 begin','y1 begin','x1 end','y1 end'均被理解为直角坐标系的定位。在map中，这些坐标值必须写作经纬度。 list(c('male',NA,100,0,5,100,5,T))可打开male系列的炫光特效。 In line, bar, k and scatter charts, 'x1 begin','y1 begin','x1 end','y1 end' are comprehended as coordinates. In map charts, these coordinates should be lattitudes and longitudes. list(c('male',NA,100,0,5,100,5,T)) opens aurora effects of series 'male'.
- markPoint: 显示标注点，默认不显示。格式写作一个列表 Show markline, default to NULL. The grammar is a list:
    - 缩略格式 Short form (长度3或4 length 3 or 4)：
    `list(c('data series 1 name|index','point name','min|max',TRUE|NULL),c('data series 2 name|index','point name','min|max',TRUE|NULL),...)`
    如list('male',,'min')或list(1,,'min')都可表示male数据系列最小值标注，只用于line, linesmooth, bar, scatter, bubble。list('male',,'min',T)则表示male系列开启炫光特效。
    ...
    - 完整格式 Full form (长度5或6 length 5 or 6): 
    ...
    如list(c('male',NA,100,0,5))表示在'male'数据系列中标注点P(0,5)。E.g., list(c('male',NA,100,0,5)) refers to a markpoint at P(0,5) as of sereis 'male'. 在line, bar, k, scatter图中，'x1','y1',...均被理解为直角坐标系的定位。在map中，这些坐标值必须写作经纬度。 list(c('male',NA,100,0,5,T))可打开male系列的炫光特效。 In line, bar, k and scatter charts, 'x1','y1',... are comprehended as coordinates. In map charts, these coordinates should be lattitudes and longitudes. list(c('male',NA,100,0,5,T)) opens aurora effects of sereis 'male'.
...
没有数据系列，因此指定显示第1个系列的均数线(`markLine=list(c(1,NA,'average'`)。
...
指定series，且显示范围从零点开始(`scale=FALSE`)。在第2个数据系列(versicolor)中打开最大(max)、最小(min)、均线(average)三根标线；在第1、2、3数据系列中分别标注max、min、max点，且打开第3个系列的眩光特效(`markPoint=list(c(1,'Max','max'),c(2,'Min','min'),c(3,'Max','max',T))`)。
...
echartR(data = iris, x = ~Sepal.Width, y = ~Petal.Width, 
        ...
        subtitle = '(source: iris)',
        xlab = 'Sepal Width', ylab = 'Petal Width',
        markLine=list(c('Linear Coef.',NA,round(fit$coefficients[[2]],2),
